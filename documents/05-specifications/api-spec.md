...
- **`position` is permanently omitted from the 201/200 response body — not a Phase 5B.3 placeholder.** Position is a function of current queue/table state (DEC-005), which at Phase 5B.3 didn't exist yet; that computation is now fully implemented (Phase 5B.13, `Allocation::QueuePositionCalculator`), but it is deliberately exposed only through `GET /guest/queue-entries/current`, never through this endpoint's own response. A value computed at join time would already be stale by the time the guest reads it (allocation/other joins can change it immediately after this response is built) — always fetching fresh via the dedicated read endpoint is the truthful design, confirmed unchanged through every phase since: the frontend's own guest join flow (`frontend/src/App.tsx`) makes exactly this second call rather than trusting a join-time number.
...
**Implementation status (Phase 5B.12, DEC-011 correctness fix):** the deferral above is now resolved. `Guest::JoinService` rejects a syntactically-valid but oversized `group_size` — checked against `Allocation::ConfigurationGenerator.maximum_seatable_group_size` (a new class method: the largest capacity across every single table or allowed adjacent pair, computed over **all** tables regardless of current occupancy — not the existing `.call`, which only considers free tables for real allocation). This is a structural ceiling, not an availability check: a group that fits some configuration but finds every suitable table currently occupied is **not** rejected — it is created normally and stays `waiting`, exactly as before. `validation_error` for this case renders the same `{ error: { type: "validation_error", message } }` shape as every other join validation failure, with a message directing the group to speak to staff. Checked after the idempotency-key lookup but before `QueueEntry.create!` — no entry, `SeatingAssignment`, or `SeatingAssignmentTable` is ever created for a rejected request, and (since nothing was persisted) a retry with the same `idempotency_key` is rejected identically, never becoming a successful idempotent replay.
...
**Position semantics (Phase 5B.4, deliberate simplification; corrected Phase 5B.13):** from Phase 5B.4 through Phase 5B.12, `position` was a chronological rank among currently-`waiting` entries only — a documented, temporary simplification, not the position model's final form (see prior revisions of this note in version control). `functional-spec.md` §9 and DEC-005 always defined the *intended* position as reflecting current table availability, compatibility, wait-time aging, and starvation-protection state (`seating-allocation-policy.md`, formalized with explicit formulas in `allocation-algorithm.md`, Phase 5B.5.1) — that computation is now wired in.

**Implementation status (Phase 5B.13, `Allocation::QueuePositionCalculator`):** `position` is computed by reusing `Allocation::DecisionEngine.decide` (unmodified) as a pure, read-only, in-memory simulation of allocation-algorithm.md §12's repeat-until-exhausted winner-selection loop — never a second scoring formula, never a database write, never `Allocation::Orchestrator`. Each simulated round asks "who would win right now," records that entry's rank, removes it and its winning configuration's table(s) from the in-memory pool, and repeats; an entry that never wins any round (no compatible configuration ever available to it, directly or because higher-priority entries consume every compatible one first) is ranked after every entry that does win a round, ordered among such leftovers by starvation status then `joined_at`. `position` is **informational only and never a guarantee of final seating order** — it is recomputed fresh on every read, from current state only, exactly as DEC-005 specifies. `GET /staff/queue` (`Staff::QueueViewService`) now reports this exact same computed position for the same entry, rather than a separate staff-only chronological rank — functional-spec.md §5 never distinguished a separate staff semantics, and DEC-005 describes one unified "position" concept.
