...
**Not yet implemented as of Phase 5B.5.3 (superseded by 25c below):** §12's `run_allocation_pass` repeat-until-exhausted loop, and wiring this service to actual allocation triggers.

## 25c. Implementation status (Phase 5B.5.4 — trigger integration + repeated allocation orchestrator)

`Allocation::Orchestrator` (`backend/app/services/allocation/orchestrator.rb`) implements §12's `run_allocation_pass` repeat-until-exhausted loop as thin composition over the unmodified `ReservationService`:

- `.call(now:)` loops calling `ReservationService.call(now:)` until it returns `:no_candidate` (normal termination — zero waiting entries, zero available configurations, or no compatible pair, per §13). Every `:success` is recorded and the loop continues immediately; no candidate list is ever cached between passes, since `ReservationService` (and therefore `ConfigurationGenerator`/`DecisionEngine`) already re-queries live database state on every single call — freshness (§6/§7/§8) is structural, not a manual refresh step that could be forgotten.
- A `:stale_candidate` result (the chosen winner lost a race before its transaction locked it) is retried, bounded by `MAX_CONSECUTIVE_STALE_RESULTS` (5, a tunable defensive constant, not a business rule) to prevent a busy-loop under genuine external contention — the counter resets to zero on every real success, so it never limits ordinary multi-allocation throughput (§19).
- Returns a `Summary` (`allocations_count`, `allocated_queue_entry_ids`, `assignment_ids`) — no internal scoring detail exposed (§21).
- `now` is captured once per `Orchestrator.call` and reused for every pass within that run — not re-read via `Time.current` per pass — so a group's aging/starvation status cannot flip mid-run purely from wall-clock drift during a fast, all-DB-round-trips loop. This is a documented implementation choice under this phase's own §8 ("if the allocation operation takes significant time, inspect the existing policy..."), not a new starvation policy — `06-ai-working-record/agent-decisions.md` Session 17 records the reasoning.

**Trigger integration (this phase's approved trigger set is Guest Join, Release, No-show, Guest Leave — §9):**
- **Guest Join** — integrated. `Guest::JoinService#create_new_entry` calls `Allocation::Orchestrator.call(now:)` immediately after a genuinely NEW entry's creation commits, then reloads the entry before returning — never on an idempotent replay or conflict outcome (those return earlier in `#call`, before this method is ever reached). A new join may therefore synchronously return `status: "ready"` in its `201` response if a compatible configuration was already available; the response *shape* (`entry_id`, `active_visit_token`, `status`) is unchanged from `api-spec.md`'s existing contract — only the `status` *value* now honestly reflects live state instead of always being `"waiting"`. See `api-spec.md`'s join section for the explicit note.
- **No-show (READY expiration path)** — integrated. `Guest::CurrentQueueStatusService#expire_if_overdue` now returns whether it actually fired; `#call` invokes `Allocation::Orchestrator` immediately after (and only after) that expiration's release transaction has committed — never nested inside it, never on an ordinary read that doesn't trigger expiration. The other no-show path (staff-initiated, manual) has no integration point because no staff no-show endpoint exists yet in this codebase.
- **Release** — **not integrated.** No staff release endpoint/service exists anywhere in this codebase yet (`api-spec.md`'s `POST /staff/seating-assignments/release` remains specification-only) — there is no committed release event to attach an orchestrator call to. Deferred to whichever future phase implements it.
- **Guest Leave** — **not integrated**, per this phase's own explicit instruction (§16/§33: "do not invent a new leave API if it has not yet been authorized... report it as deferred"). `POST /guest/queue-entries/current/leave` has never been implemented (deferred every phase since 5B.3) and implementing it is outside this phase's scope (allocation-trigger integration, not new guest-facing API surface). Deferred to whichever future phase implements the leave endpoint itself.

**Verified with real end-to-end tests, not mocks:** `backend/test/services/allocation/orchestrator_test.rb` (multi-allocation with an exact 1:1:1 capacity match, scarcity recomputation proven with exact before/after numbers, combined-configuration recomputation proven by asserting the correct final table assignment for both a group that can only use the combined pair and one that falls back to a freed single table, starvation override effective through the orchestrator, and a bulk termination test); `backend/test/controllers/guest/queue_entries_allocation_integration_test.rb` (real HTTP: join synchronously returns `ready`, idempotent retry never double-allocates, multiple joins never double-book a table); `backend/test/services/guest/current_queue_status_allocation_integration_test.rb` (a lazy no-show expiration correctly fills the newly-freed table for a different waiting group; a non-overdue read never triggers allocation).
