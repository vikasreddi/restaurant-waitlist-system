...
- Prompt 25 (below) authorized a frontend-only fix for a session/visit persistence gap discovered during real browser verification — Guest active-visit and Staff-login state both survive a browser refresh via `localStorage`, recovered/validated through the existing real backend endpoints, no backend contract changed — supplied complete as `P0_Frontend_Session_Persistence_Recovery_Claude_Prompt(1).md`.
- Prompt 26 (below) authorized the P0 Staff Table View vertical slice — `GET /staff/tables`, a read-only view deriving `free`/`held`/`occupied` state from existing `SeatingAssignmentTable`/`SeatingAssignment` data (explicitly forbidding any stored `tables.status` column), plus a minimal frontend screen — explicitly forbidding Staff Release/No-show/table-editing and requiring a short plan before implementation — supplied complete as `P0_Staff_Table_View_Claude_Prompt(1).md`.
- Prompt 27 (below) authorized three bundled P0 Staff operational capabilities in one task — completing the frontend for the already-existing `POST /staff/seat`, and building both backend + frontend for Staff Release and Staff No-show for the first time — explicitly forbidding any new authentication/session infrastructure, a `whoami` endpoint, or a frontend state-management library, and requiring reuse of every existing Staff/allocation/persistence mechanism — supplied complete as `P0_Staff_Seat_Release_NoShow_Claude_Prompt.md`.
- Prompt 28 (below) authorized a DEC-011 correctness fix — reject a Guest Join whose group size no valid table configuration could ever seat, derived from existing configuration logic (never hardcoded), explicitly distinguishing "structurally impossible" from "currently unavailable" (the latter must still join and wait) — supplied complete as `P0_Oversized_Group_Rejection_Claude_Prompt(1).md`.
- Prompt 29 (below) authorized a DEC-005 correctness fix — replace the guest-facing `position`'s chronological-only approximation (documented as a temporary simplification since Phase 5B.4) with the real compatibility/availability/aging/starvation-aware rank, reusing `Allocation::DecisionEngine` unmodified rather than inventing a second scoring formula, and requiring Staff Queue to be updated only if the same semantics genuinely apply — supplied complete as `P0_Guest_Dynamic_Position_Claude_Prompt(1).md`.
- Prompt 30 (below) authorized the P0 Final Audit — a comprehensive re-verification of every P0 requirement, hard path, API contract, frontend↔backend integration, database invariant, and documentation statement built across Prompts 1–29, explicitly forbidding new feature work and requiring corrections only for genuine gaps found — supplied complete as `P0_Final_Audit_Claude_Prompt(1).md`.
...
## Prompt 25 — P0 Task: Frontend Session Persistence & Recovery

**Source:** `P0_Frontend_Session_Persistence_Recovery_Claude_Prompt(1).md` (candidate-authored, read from `~/Downloads/`), delivered complete in a single message. Frames itself explicitly as a bug fix ("the frontend session/visit persistence gap discovered during real browser verification"), not a new feature — the current state list matches the prior commit (`3f35f0b feat: implement staff queue`) exactly.

**Objective (verbatim intent):** implement ONLY (1) Guest active-visit persistence/recovery after refresh, (2) Staff authentication persistence/recovery after refresh. Explicitly forbids Staff Table/Release/No-show/Redis/Sidekiq/notifications/live updates/P1 work.

**Full text (condensed; all numbered sections' substantive content preserved):**

> **Current state.** Already implemented: Guest Join/Current/Leave APIs, allocation engine + transactional allocation + orchestration, Staff Login API + authentication, Staff Seating Confirmation API, Guest Join/Leave UI, Staff Queue API + UI, real PostgreSQL/API/browser verification. Existing contracts: `GET /guest/queue-entries/current` via `Authorization: Bearer <active_visit_token>`; the existing Staff authentication/session mechanism and `Staff::BaseController` — do NOT create a new authentication system.
>
> **§1 First — inspect.** Read `CLAUDE.md`, current `frontend/src/App.tsx` and all existing frontend components, current Guest Join/Leave implementation, current Staff Login implementation, current Staff Queue implementation, current token/session handling, `api-spec.md`, `functional-requirements.md`, `traceability.md`, existing frontend tests/tooling if any. Determine exactly how Guest and Staff tokens are currently stored and restored. Do not assume the architecture.
>
> **§2 Guest visit persistence.** The current Guest UI keeps `active_visit_token` only in React/component state — change this so the active visit survives a browser refresh. Use a simple browser-local persistence mechanism (e.g. `localStorage`) unless the repository/specification requires otherwise. Persist ONLY the information necessary to recover the active guest visit — never phone number, group size, or idempotency key as identity. The persisted identity must be the existing `active_visit_token`.
>
> **§3 Guest recovery.** On initialization: no stored token → normal Join screen. A stored token → call `GET /guest/queue-entries/current` with `Authorization: Bearer <active_visit_token>` and restore the UI from the real response. Do NOT fabricate state locally — the backend remains the source of truth.
>
> **§4 Guest recovery states.** WAITING → restore state + real position. READY → restore state + real seating_code. SEATED → restore existing terminal-state UI. LEFT/NO_SHOW → restore existing terminal-state UI per the current API contract. 404/invalid token → remove the stored token, return to Join screen, do not auto-create a new entry. Network/API error → do NOT immediately delete a valid stored token merely because of a temporary error; show an appropriate error/retry state.
>
> **§5 Guest Join/Leave token lifecycle.** Verify the existing lifecycle. Successful Join (backend returns `active_visit_token`) → persist it. Idempotent Join retry → do not replace the active visit token incorrectly. Leave: after success, clear the active visit token, show the existing LEFT state, do not automatically create another visit. Terminal state: follow the existing product behavior — do not invent a new guest-session lifecycle.
>
> **§6 Staff session persistence.** The current Staff Login also loses authentication after refresh — fix using the existing Staff token/session mechanism. Persist the existing Staff authentication token using an appropriate browser-local mechanism. Do NOT create refresh tokens, OAuth, SSO, MFA, backend cookie/session infrastructure, a new authentication mechanism, or a new database table. The backend authentication contract must remain unchanged.
>
> **§7 Staff recovery.** On initialization: no Staff token → existing Staff Login screen. A Staff token exists → restore the authenticated Staff state and allow the Staff UI to load; the Staff Queue should then call the existing `GET /staff/queue` using the existing Staff authentication mechanism. Do not fabricate an authenticated state without validating the token through the backend. If the backend returns 401: clear the stored Staff token, return to Staff Login, show the appropriate authentication state. Do not treat a temporary network error as proof the token is invalid.
>
> **§8 Guest/Staff separation.** Must remain completely separate — do not accidentally allow `active_visit_token` to authenticate Staff, or the Staff token to be used for Guest APIs. Use separate storage keys, e.g. `guest_active_visit_token` / `staff_session_token`. Use names consistent with the existing frontend architecture's convention if one exists.
>
> **§9 Tab/toggle behavior.** Preserve the existing Guest/Staff switching behavior. Switching Guest → Staff must not delete the Guest's active visit. Switching Staff → Guest must not log the Staff user out. Stored session/visit state should survive navigation between the two views.
>
> **§10 Loading/initialization.** Avoid rendering the wrong screen briefly during recovery: application starts → restore persisted token(s) → validate/recover through backend → render correct Guest/Staff state. Do not show "Join Queue" for a moment and then unexpectedly replace it with an existing visit unless unavoidable. Keep the implementation simple — do not introduce a global state framework just for this task.
>
> **§11 Security/scope.** Local P0 MVP only — do not claim `localStorage` provides production-grade credential security, do not add production authentication infrastructure. Never expose tokens in console logs, UI, error messages, documentation, screenshots, or the final report. Do not commit secrets.
>
> **§12 Tests.** Use existing frontend testing/tooling if present; do not introduce a large new framework. At minimum verify through real browser testing: Guest — no token→Join screen; Join→token persisted; READY→refresh→same READY+code; WAITING→refresh→same WAITING, same entry, no duplicate; Leave→token cleared, refresh after Leave doesn't create a new visit; invalid token→cleared+Join screen; temporary API/network error doesn't unnecessarily clear the token. Staff — Login→token persisted; refresh→still authenticated; Staff Queue loads after refresh; invalid Staff token→logout/login screen; temporary network error doesn't unnecessarily clear the token. Separation — Guest token can't authenticate Staff; Staff token can't be used as Guest `active_visit_token`; switching Guest/Staff doesn't destroy the other's persisted state.
>
> **§13 Real browser verification.** Not complete based on code inspection alone — use the real running frontend/backend. Flow A (Guest READY refresh): join size 2 → READY → note seating code → refresh → confirm READY + identical seating code + no new QueueEntry. Flow B (Guest WAITING refresh): join a group that can't currently fit → WAITING → refresh → confirm still WAITING, position from backend, no duplicate. Flow C (Guest Leave): active visit → Leave → LEFT → refresh → confirm no automatic new Join. Flow D (Staff refresh): Login → Staff Queue loads → refresh → confirm still authenticated → Staff Queue loads again from real API data. Flow E (invalid authentication): verify invalid/expired token handling using existing backend behavior — do not expose actual tokens in the report.
>
> **§14 Database verification.** Inspect real PostgreSQL after the browser flows. Confirm: READY entry id and seating_code unchanged after Guest refresh; no duplicate QueueEntry; WAITING entry unchanged after refresh; Leave doesn't create another QueueEntry; Staff refresh doesn't change database state; 40 tables and 19 adjacency pairs intact. Clean only disposable data created for verification — do not delete unrelated pre-existing rows.
>
> **§15 Do not change backend contracts.** Do not modify the Guest Join/Current/Leave, Staff Login, Staff Queue, or Staff Seating API contracts, the allocation algorithm, or the database schema — unless a genuine existing defect makes persistence/recovery impossible. If a backend change appears necessary: STOP and report the exact dependency, do not silently expand scope.
>
> **§16 Documentation.** Update only documentation that actually needs to record this P0 frontend behavior — likely `api-spec.md` only if API behavior is affected, `traceability.md`, relevant AI working-record files. Do not rewrite unrelated specifications.
>
> **§17 Strictly do NOT implement:** Staff Table, Staff Release, Staff No-show, Staff management, registration, roles/permissions, OAuth, SSO, MFA, refresh-token infrastructure, Redis, Sidekiq, WebSockets, notifications, live updates, analytics, P1 fairness, AI runtime decisions, unrelated frontend redesign, new backend APIs. This task is ONLY: Guest visit persistence/recovery + Staff login persistence/recovery.
>
> **§18 Git.** Before committing: `git status`, `git diff --stat`, `git diff -- <relevant files>`. Run `npx tsc -b --noEmit` and the existing frontend lint command if applicable; run the full backend suite to ensure no regression. Create ONE focused commit: `feat: persist guest and staff frontend sessions`. Do NOT push automatically. Leave the working tree clean.
>
> **§19 Final response:** Status (PASS/BLOCKED); Guest Persistence (what changed, how recovery works); Staff Persistence (what changed, how recovery works); Tests (frontend checks + backend full suite); Real Browser Verification (exact refresh/recovery flows tested); Database Verification (duplicate-entry and persistence checks); Documentation (files changed); Git (commit hash, working-tree status); Scope (confirm Staff Table, Staff Release, Staff No-show, Redis, Sidekiq, notifications, live updates, and P1 work were NOT implemented). "STOP HERE."

---

## Prompt 26 — P0 Task: Staff Table View Vertical Slice

**Source:** `P0_Staff_Table_View_Claude_Prompt(1).md` (candidate-authored, read from `~/Downloads/`), delivered complete in a single message. Current-checkpoint list matches the prior commit (`fe42663 feat: persist guest and staff frontend sessions`) exactly, and notes Staff Queue has already been manually browser-verified.

**Full text (condensed; all numbered sections' substantive content preserved):**

> **§1 Before coding — inspect.** Read `CLAUDE.md`, `functional-requirements.md`, `traceability.md`, `api-spec.md`, `functional-spec.md`, `allocation-spec.md`, `allocation-algorithm.md`, `table.rb`, `seating_assignment.rb`, `seating_assignment_table.rb`, `queue_entry.rb`, existing Staff controllers/services, existing Staff authentication, existing Staff Queue implementation, current routes, current frontend Staff components. Determine the exact P0 Staff Table requirements from the repository — do not invent requirements if the specifications already define them.
>
> **§2 First — give a short plan.** Before modifying files, state: the exact Staff Table endpoint required by the specification; the authentication requirement; the exact response shape; how table state is derived; the required Staff Table UI; files to change. Then proceed with implementation — do not ask unnecessary clarification questions.
>
> **§3 Core requirement.** Implement a read-only Staff Table view answering "Which restaurant tables exist and what is their current operational state?" Use the real PostgreSQL database. Do not hardcode table data. The existing seed contains 40 tables (20×2-seat, 18×4-seat, 2×6-seat) and 19 adjacency pairs — the view must reflect the actual database.
>
> **§4 Very important — do NOT add table status.** Do NOT add `tables.status` or `available`/`occupied`/`reserved`/`released` as a persisted `Table` column. State must be derived from the existing domain model via `Table → SeatingAssignmentTable → SeatingAssignment → QueueEntry`. The existing table-exclusivity constraint (`UNIQUE(table_id) WHERE released_at IS NULL`) remains authoritative and must not change.
>
> **§5 Define table state from existing data.** Follow the authoritative specification. For each table, determine its current state from real assignment data — at minimum distinguish `AVAILABLE`/`RESERVED`-`PENDING`/`OCCUPIED`-`ACTIVE` (or whatever P0 states the specification requires). Do not invent additional business states. Use the existing `SeatingAssignment.status` and `SeatingAssignmentTable.released_at` semantics — historical released rows remain historical; a table with only historical released rows is available; a table with a current unreleased assignment is unavailable per that assignment's status. If the specification uses different terminology, follow the specification exactly.
>
> **§6 Staff authentication.** Use the existing mechanism — the endpoint must inherit/use `Staff::BaseController` or the existing equivalent used by Staff Queue. Do NOT create another authentication mechanism. Verify: valid token → 200; missing token → 401; invalid token → 401; Guest `active_visit_token` → 401. Do not expose tokens in responses or UI.
>
> **§7 Backend API.** Implement the exact endpoint already specified in `api-spec.md` — do NOT invent a different route. Read-only. Return only the fields required by the specification (potentially `table_id`, `table_name`, `capacity`, `state`, current-assignment/queue-entry information if explicitly required) — use the exact specification rather than blindly adding fields. Do not return password information, authentication tokens, internal secrets, or unnecessary database metadata.
>
> **§8 Read-only behavior.** A normal GET must NOT create a `QueueEntry`/`SeatingAssignment`/`SeatingAssignmentTable`, allocate, release, modify a table, change `QueueEntry`/`SeatingAssignment` state, or generate seating codes. If the authoritative specification explicitly requires DEC-015 lazy expiration as part of this endpoint, reuse the existing `LazyExpiration` mechanism — do not duplicate expiration logic. Do not trigger the allocation orchestrator from this read unless the specification explicitly requires it.
>
> **§9 Query design.** Use deliberate database queries; avoid N+1. Do not introduce Redis, caching, Sidekiq, background jobs, new persistence tables, or premature indexes. Use existing associations/indexes; prefer ActiveRecord/preloading over unnecessary raw SQL. Do not optimize for hypothetical production scale — correctness and clarity matter more for this P0.
>
> **§10 Frontend — Staff Table view.** Add the screen using the existing frontend architecture — do not redesign the application. Use the existing Staff authenticated experience. Simple and operational (illustrative mockup: "Staff / [ Queue ] [ Tables ] / Tables / Table 1  2 seats  AVAILABLE / ..."). Use the exact terminology defined by the project specification. Display real API data — do NOT hardcode 40 tables, capacities, states, assignment IDs, or guest IDs.
>
> **§11 Frontend states.** Loading (clear loading state). Loaded (show all required tables). Empty (handle cleanly if the specification permits zero tables — do not fake an empty state if the database has tables). Error (user-readable). Authentication failure (use the existing Staff session recovery behavior — if the API returns 401, follow existing handling; do not create another auth flow).
>
> **§12 Refresh/session persistence.** Preserve the previous phase's Staff session persistence. Verify: Staff Login → Staff Table → browser refresh → Staff remains authenticated → Staff Table reloads from real API. Do not regress the Guest persistence behavior.
>
> **§13 Required test scenarios.** Add focused backend tests, at minimum where applicable: valid/missing/invalid Staff authentication; Guest token rejected; all seeded tables returned; correct table IDs/capacities; available/pending-reserved/active-occupied state; a released historical assignment does not make a table appear occupied; multiple tables reserved by one combined assignment represented correctly; no duplicate table records; correct response shape; no database mutation caused by GET; no new `SeatingAssignment`/`SeatingAssignmentTable` created; existing allocation behavior unchanged. Run the full backend suite. Do not introduce a new testing framework.
>
> **§14 Important domain test.** Explicitly test: table → old released assignment → no current unreleased assignment → table must be AVAILABLE; and: table → current unreleased assignment → table must NOT be AVAILABLE. Important because historical assignment rows must never be deleted.
>
> **§15 Combined table test.** Verify a two-table `SeatingAssignment` shows both tables consistently in the Staff Table view. Do not create any new combined-table logic — reuse existing assignment records.
>
> **§16 Real HTTP verification.** Run the real backend with a real authenticated Staff session. Verify `GET /staff/<exact-table-route>` → 200 → real PostgreSQL table data; no `Authorization` → 401; Guest `active_visit_token` → 401. Do not include real tokens in the final report.
>
> **§17 Real database verification.** Inspect PostgreSQL directly. Verify 40 tables present, 20 capacity-2, 18 capacity-4, 2 capacity-6, 19 adjacency pairs remain; currently reserved/occupied tables match assignment records; released tables are available; historical released rows remain; no unexpected records were created. Create disposable assignments only if necessary; clean up only data created by this task; do not delete pre-existing rows.
>
> **§18 Real browser verification.** Flow A (baseline): Login as Staff, open Tables, confirm real tables displayed. Flow B (create READY guest): use the existing Guest flow to create a real READY guest, return to Staff Tables, confirm the table used changes state. Flow C (Staff confirmation): if Staff Seating Confirmation is already available, confirm a READY guest using the existing flow, return to Staff Tables, confirm the same table reflects the new state — do NOT implement new confirmation logic. Flow D (release behavior): do NOT implement Staff Release in this task; if no existing release operation exists, do not manufacture one just to test this — use existing test/database mechanisms only if needed to verify released rows are treated correctly. Flow E (browser refresh): refresh the Staff Table page, confirm Staff remains authenticated, tables reload, states still match PostgreSQL.
>
> **§19 Frontend regression check.** Because this screen is added after Guest/Staff session persistence, verify: Guest Join → refresh → same visit; Staff Login → refresh → authenticated; Staff Queue → still works. Do not regress these existing flows.
>
> **§20 Documentation.** Update only relevant authoritative documentation — likely `api-spec.md`, `traceability.md`. Follow the existing AI working-record convention (`agent-prompts.md`, `session-log.md`, `agent-decisions.md`). Only add an `ai-corrections.md` entry for a genuine correction. Do not rewrite unrelated historical documentation.
>
> **§21 Strict scope — do NOT implement:** Staff Release, Staff No-show, Staff Table create/update/delete, table configuration UI, table editing, Staff management, registration, roles/permissions, OAuth, SSO, MFA, password reset, refresh-token infrastructure, Redis, Sidekiq, WebSockets, notifications, live updates, rate limiting, analytics, P1 fairness, missed-opportunity tracking, fairness debt, predictive demand, AI runtime decisions, allocation algorithm changes, new table status database column, new database tables, unrelated frontend redesign. If another feature appears necessary: STOP and report the dependency instead of implementing it.
>
> **§22 Git.** Before committing: `git status`, `git diff --stat`, `git diff -- <relevant files>`. Run `npx tsc -b --noEmit`, frontend lint if configured, and the FULL backend test suite. Create ONE focused commit: `feat: implement staff table view`. Do NOT push automatically. Leave the working tree clean.
>
> **§23 Final response:** Status (PASS/BLOCKED); API (exact endpoint and response behavior); Backend (service/query/controller implementation); Table State (explain how AVAILABLE/RESERVED/OCCUPIED are derived, explicitly confirm no `tables.status` column was added); Frontend (Staff Table UI and states implemented); Tests (new tests, full-suite result); Real Verification (API + browser + PostgreSQL); Documentation (files changed); Git (commit hash, working-tree status); Scope (confirm Staff Release, Staff No-show, Redis, Sidekiq, notifications, live updates, and P1 functionality were NOT implemented). "STOP HERE. Do not start another phase."

---

## Prompt 27 — P0 Task: Staff Seat UI + Staff Release + Staff No-show

**Source:** `P0_Staff_Seat_Release_NoShow_Claude_Prompt.md` (candidate-authored, read from `~/Downloads/`), delivered complete in a single message. No plan-first requirement this time (unlike Prompts 24/26) — proceeded directly from inspection into implementation. Three P0 Staff operational capabilities bundled into one task: completing the frontend for the already-existing `POST /staff/seat`, and building both backend + frontend for Release and No-show for the first time.

**Full text (condensed; all numbered sections' substantive content preserved):**

> **Objective.** Implement ONLY: (1) Staff Seat by seating code — complete the existing backend capability with the missing Staff UI; (2) Staff Release — backend + Staff UI; (3) Staff No-show — backend + Staff UI. Do NOT implement anything outside these three capabilities.
>
> **§1 Staff Seat by Seating Code.** `POST /staff/seat` already exists and requires Staff authentication — complete the frontend flow. Keep the existing Queue/Tables navigation. Add a clear "Seat Guest" action/section with a `seating_code` input and a "Seat" button, using the existing Staff session token, calling the real endpoint, never fabricating success locally. Handle loading, success, invalid/unknown code, already-used/non-ready code, expired reservation, and network/server errors. On successful seating, refresh Staff Queue and Staff Tables. Do not rerun allocation when staff seats a guest — staff only confirms the existing reservation.
>
> **§2 Staff Release.** Implement per the existing project specification. Backend: add the appropriate Staff endpoint/service/controller. The operation must require Staff authentication, identify the seated group by `queue_entry_id`, resolve its current `SeatingAssignment` internally, transition the assignment `active → released`, transition `QueueEntry` from `seated` to the appropriate existing terminal/released state, set `released_at` on ALL associated `SeatingAssignmentTable` rows, never delete historical rows. If one assignment contains two adjacent tables, release both atomically. Do not create a second allocation implementation. After successful release, invoke the existing `Allocation::Orchestrator` using the established project pattern. Already-released/terminal targets must not corrupt state or create duplicate releases. Do NOT release by raw `table_id` — the backend must resolve the complete assignment from the queue entry. UI: add the smallest reasonable Release action for seated guests; after release, show success, refresh Queue, refresh Tables.
>
> **§3 Staff No-show.** Backend: add the appropriate Staff endpoint/service/controller. Require Staff authentication, accept `queue_entry_id`, support WAITING and READY, transition `QueueEntry` to `no_show`, make `no_show` terminal, prevent later seating. WAITING → `no_show` (no seating assignment should exist). READY → `no_show`, releasing the existing `SeatingAssignment` in the SAME transaction (assignment → released, table claim row(s) → `released_at` set, both atomically if combined). After tables become available, invoke the existing `Allocation::Orchestrator`. Do not create a new allocation mechanism.
>
> **§4 Staff UI.** Extend the existing Staff UI only — do not redesign it. Queue: for WAITING entries show position, group size, and a "No-show" action; for READY entries show group size, seating code, and "Seat"/"No-show" actions; use real backend APIs. Tables: keep the existing view; after Seat, Release, or No-show, refresh Queue/Tables from the real backend; never represent occupancy using fake/local table state.
>
> **§5 Errors and authentication.** Use the existing Staff authentication (`Authorization: Bearer <staff-token>`) — missing/invalid/guest token must return the existing 401 behavior. Use existing API error conventions for: unknown seating code, code not ready, expired reservation, already seated, invalid release target, invalid no-show target. Do not expose unnecessary internal database details.
>
> **§6 Tests.** Seat: valid READY code → seated; invalid code rejected; already-used code rejected; expired READY behavior; authentication required. Release: seated group releases successfully; table becomes FREE; allocation is triggered; combined assignment releases BOTH tables; historical assignment rows remain; no partial combined release; authentication required. No-show: waiting → no_show; ready → no_show; ready no-show releases its table; combined ready assignment releases BOTH tables; no_show is terminal; no_show cannot later be seated; authentication required. Run the full backend suite and existing frontend type/lint checks. Do not weaken or delete existing tests.
>
> **§7 Real verification.** Use the real Docker/PostgreSQL application. Seat: guest joins → READY → Staff sees READY → Staff enters code → Seat → guest becomes SEATED → table becomes OCCUPIED. Release: SEATED → Staff Release → assignment released → table FREE → allocation can use it again. Combined release: large group → two adjacent tables → SEATED → Release → both tables FREE. Waiting no-show: WAITING → Staff No-show → NO_SHOW → cannot later be seated. Ready no-show: READY → Staff No-show → NO_SHOW → reservation released → table(s) FREE → allocation can proceed. Use disposable data and clean it up; do not modify unrelated pre-existing data.
>
> **§8 Documentation.** Update only documentation required for these three capabilities — likely `api-spec.md`, `traceability.md`, existing AI working-record files per project convention. Do not rewrite unrelated documentation.
>
> **§9 Strict no-overengineering boundaries.** DO NOT implement: Staff management, roles/permissions, OAuth/SSO/MFA, registration/password reset, refresh-token infrastructure, Redis, Sidekiq, WebSockets, live updates, notifications, rate limiting, analytics, table creation/edit/delete/configuration, camera/QR scanning, new authentication architecture, new session system, `whoami` endpoint, frontend routing/state-management libraries, unrelated UI redesign, unrelated refactoring, new allocation algorithm, P1 functionality. Reuse the existing: Staff authentication, `Staff::BaseController`, `Staff::SessionToken`, `Allocation::Orchestrator`, domain models, transaction/locking patterns, frontend session persistence, Queue/Tables components, API error conventions. Do not duplicate existing logic.
>
> **§10 Architecture rules.** Allocation is performed by the system, not by staff. Lifecycle: `WAITING -> Allocation::Orchestrator -> READY -> Staff enters seating code -> SEATED -> Staff Release -> RELEASED`. No-show: `WAITING -> NO_SHOW` or `READY -> NO_SHOW -> release reservation`. Staff does not choose a table when seating — staff only confirms an existing reservation. Do not change this architecture.
>
> **§11 Completion criteria.** Do not declare complete until: Staff can seat a READY guest through the UI; Staff can release a seated guest through the UI; Staff can mark WAITING/READY guests as no-show through the UI; combined assignments release atomically; allocation is triggered correctly after released tables become available; authentication is enforced; tests pass; real Docker/PostgreSQL verification passes; existing Guest Join/Leave/refresh behavior still works; existing Staff Login/Queue/Tables/refresh behavior still works; working tree contains only changes related to this task. Provide a concise implementation summary, tests, real verification, files changed, and commit SHA. "STOP after this task. Do not start the next P0 phase."

---

## Prompt 28 — P0 Task: Oversized Group Rejection (DEC-011)

**Source:** `P0_Oversized_Group_Rejection_Claude_Prompt(1).md` (candidate-authored, read from `~/Downloads/`), delivered complete in a single message. A correctness fix, not a new feature — closes a deferral `api-spec.md` has explicitly documented since Phase 5B.3 ("The DEC-011 'group size exceeds every seatable configuration' rejection is also deferred... allocation-adjacent logic not yet built").

**Objective:** a Guest Join request must be rejected when the requested group size can NEVER be seated by any valid table configuration — the maximum must be derived from existing configuration logic, never hardcoded.

**Full text (condensed; all numbered sections' substantive content preserved):**

> **§1 Inspect before editing.** Read `CLAUDE.md`, `functional-requirements.md`, `traceability.md`, `functional-spec.md`, `allocation-spec.md`, `allocation-algorithm.md`, `api-spec.md`. Inspect the existing `Guest::JoinService`, `Allocation::ConfigurationGenerator`, `Allocation::DecisionEngine`, existing join validation, existing configuration tests, existing Guest Join controller/tests. Determine the authoritative rule for a group that can never fit — do not invent a new business rule if the specification already defines it.
>
> **§2 Required behavior.** A valid group: normal validation → `QueueEntry` created → existing allocation flow. An impossible group: determine no valid single/allowed-combined configuration can ever fit → reject → NO `QueueEntry`/`SeatingAssignment`/`SeatingAssignmentTable` created, NO allocation triggered. The rejection must happen before creating the `QueueEntry`.
>
> **§3 Important distinction.** Do NOT reject a group merely because tables are currently occupied — a group that fits a valid configuration but finds every suitable table currently unavailable is still valid and must remain eligible for WAITING. Only reject when `group size > maximum size of every valid configuration`. Example: if the largest valid configuration is 8, `group_size = 8` is valid (may become READY or WAITING depending on availability); `group_size = 9` is impossible (rejected, no `QueueEntry`). Do not assume 8 is the actual maximum — derive it from the existing configuration model.
>
> **§4 Reuse existing configuration logic.** Prefer reusing `Allocation::ConfigurationGenerator` or another already-authoritative configuration source. Do NOT duplicate the table-capacity calculation, hardcode the maximum, create a second configuration algorithm, modify `DecisionEngine` unnecessarily, modify table schema, or add a database column. There must be one source of truth for valid seating configurations. If the existing configuration generator is not suitable for validation, make the smallest clean refactor necessary so the same domain logic can answer "What is the maximum group size that can ever be seated?" — keep the refactor tightly scoped.
>
> **§5 API behavior.** Follow existing API error conventions — inspect `api-spec.md` before deciding the exact status/error shape. The response should clearly indicate the requested group size cannot be accommodated by any valid configuration. Do not expose internal configuration details unless the existing specification requires them. Do not change unrelated API response shapes.
>
> **§6 Idempotency.** An impossible join must not create a `QueueEntry`, and therefore must not create a persistent idempotency record/state that could later interfere with a valid retry. Follow the existing idempotency design. Test that an impossible request stays rejected on retry with the same request, with no `QueueEntry` created either time — do not accidentally turn an impossible request into a successful idempotent replay.
>
> **§7 Validation order.** Preserve existing validation behavior. At minimum: malformed group size remains a validation error; non-positive group size remains invalid; impossible-but-syntactically-valid group size gets the new impossible-group error; currently-unavailable-but-valid group size remains WAITING/normal behavior. Do not replace existing validation errors unnecessarily.
>
> **§8 Tests.** Service tests, at minimum: valid maximum-sized group accepted; group larger than maximum rejected; impossible group creates no `QueueEntry`/`SeatingAssignment`/`SeatingAssignmentTable`; impossible group does not trigger allocation; currently-unavailable-but-valid-sized group NOT rejected as impossible; same impossible request stays rejected on retry; existing valid/idempotent join behavior unchanged. Controller tests: correct HTTP status; correct error type/message per the existing API spec; no `QueueEntry` created. Regression: run the complete backend suite — do not weaken or delete existing tests.
>
> **§9 Real API verification.** Use the real running Rails/PostgreSQL application. Determine the actual maximum valid group size from the application's configuration, then test: maximum valid → accepted, normal WAITING/READY behavior; one above maximum → rejected, no `QueueEntry`; much larger → rejected, no `QueueEntry`; valid but unavailable (occupy the relevant tables) → WAITING, must NOT be rejected as impossible. Clean all disposable data afterward — do not modify/delete unrelated pre-existing data.
>
> **§10 Database verification.** After the impossible-group request, verify `QueueEntry`/`SeatingAssignment`/`SeatingAssignmentTable` counts unchanged — the request must be side-effect free. Confirm the existing 40 tables / 19 adjacency pairs remain intact.
>
> **§11 Frontend.** Do NOT redesign the frontend. If the existing Guest Join UI already displays backend validation errors, make only the minimum change necessary for the new API error to be visible. Do not add new UI components or dependencies. If no frontend change is necessary, leave the frontend untouched.
>
> **§12 Documentation.** Update only documentation that needs to reflect the corrected P0 behavior — likely `api-spec.md` (if the error contract needs documentation), `traceability.md`, existing AI working-record files per project convention. Do not rewrite unrelated documentation.
>
> **§13 Strict scope.** DO NOT implement: dynamic guest position, Staff functionality, Staff Release/No-show changes, allocation algorithm redesign, new allocation logic, Redis, Sidekiq, WebSockets, notifications, live updates, analytics, fairness/P1 features, AI runtime allocation, new database tables, new database columns, new authentication, unrelated refactoring, frontend redesign. This task is ONLY: reject impossible group sizes before `QueueEntry` creation.
>
> **§14 Git.** Before committing: `git status`, `git diff --stat`, `git diff -- <relevant files>`. Run `npx tsc -b --noEmit` if frontend files changed. Run the full backend suite. Create ONE focused commit: `fix: reject impossible guest group sizes`. Do NOT push automatically. Leave the working tree clean.
>
> **§15 Final response:** report concisely — maximum valid group size as derived from existing configuration; where validation was implemented; API behavior; tests added; full test result; real API/database verification; documentation changed; commit SHA; confirmation that no `QueueEntry`/assignment is created for impossible groups; confirmation that valid-but-currently-unavailable groups still enter WAITING. "STOP HERE. Do not implement the next P0 task."

---

## Prompt 29 — P0 Task: Spec-Compliant Guest Dynamic Queue Position

**Source:** `P0_Guest_Dynamic_Position_Claude_Prompt(1).md` (candidate-authored, read from `~/Downloads/`), delivered complete in a single message. A correctness fix: the guest-facing `position` had been a documented, deliberate simplification (chronological rank only) since Phase 5B.4, explicitly flagged there as "not the position model's final form."

**Objective:** change guest-facing `position` to reflect the authoritative queue-position rules (currently waiting groups, current table/configuration availability, group/table compatibility, wait time/aging, starvation protection) — using existing specifications and allocation concepts as the source of truth, never inventing a new ranking formula.

**Full text (condensed; all numbered sections' substantive content preserved):**

> **§1 Read before coding.** Read `CLAUDE.md`, `functional-requirements.md`, `traceability.md`, `functional-spec.md`, `allocation-spec.md`, `allocation-algorithm.md`, `api-spec.md`, the relevant DEC-005/queue-position decision documentation, `Allocation::ConfigurationGenerator`, `Allocation::DecisionEngine`, `Allocation::Policy`, `Guest::CurrentQueueStatusService`, `Staff::QueueViewService`, existing queue-position tests, existing allocation tests. Determine the exact authoritative definition of guest `position` — do NOT invent a new ranking formula.
>
> **§2 Critical architectural rule.** Do NOT duplicate the allocation algorithm inside `Guest::CurrentQueueStatusService`. The existing allocation system already owns candidate generation, compatibility, scarcity, starvation, scoring, table matching — reuse existing domain logic. If a small shared extraction is required, make it reusable and keep the allocation algorithm itself unchanged. Do NOT change `Allocation::DecisionEngine` scoring semantics, `Allocation::ReservationService`, `Allocation::Orchestrator`, table-locking behavior, or allocation candidate tie-break rules, unless the existing specification proves a bug is required. This task is about the reported queue position, not changing actual allocation behavior.
>
> **§3 Required semantics.** For a WAITING guest, `position` must represent its rank among currently waiting guests per the authoritative P0 queue-position rules — accounting for availability (current free table/configuration state matters; a currently-servable group should not necessarily share position with one that can't currently be served), compatibility (existing configuration generator/compatibility rules), wait time/aging (existing policy, no second aging formula), and starvation protection (existing threshold/policy, no duplicated/redefined constant).
>
> **§4 Important distinction.** Do NOT interpret position as "who joined first?" unless the authoritative specification says those two rankings happen to be identical for the current state. The current chronological-count implementation is precisely the behavior that needs to be evaluated/replaced.
>
> **§5 Position must be informational.** The position endpoint must NOT reserve a table, create a `SeatingAssignment`/`SeatingAssignmentTable`, modify `QueueEntry` state, trigger allocation, or mutate table state — it is a read. Do not call `Allocation::Orchestrator` merely to calculate position; do not create a fake reservation to determine position.
>
> **§6 Guest current status API.** Keep the existing endpoint (`GET /guest/queue-entries/current`, `Authorization: Bearer <active_visit_token>`) and response shape unless the specification explicitly requires a change. For WAITING: `{ entry_id, status: "waiting", position }`. For READY/terminal states, preserve existing behavior. Do not change seating-code behavior or guest session persistence.
>
> **§7 Staff Queue.** Inspect the specification to determine whether Staff Queue's displayed position must use the same dynamic semantics. If required, update `Staff::QueueViewService` to use the same shared position calculation; if Staff Queue intentionally has different semantics, leave it unchanged and document why. Do not blindly change Staff behavior.
>
> **§8 Efficiency.** Do not introduce an N+1 query — prefer one snapshot of waiting entries, one snapshot of relevant table/configuration state, reusable domain calculation. Do not introduce Redis, caching infrastructure, background jobs, Sidekiq, new database tables, unnecessary indexes, or speculative production optimization. The goal is a clear, correct P0 implementation.
>
> **§9 Tests.** Add focused tests proving the difference between chronological order and actual position, at minimum: Scenario A (chronological order differs from eligibility — Group A joined earlier but is not currently serviceable, Group B joined later but is; position follows the spec, not `joined_at`); Scenario B (same eligibility — tie-break/aging rule); Scenario C (starvation — a starvation-protected guest receives the priority the existing policy defines); Scenario D (availability changes — changing current table availability changes the reported position, without modifying allocation state merely to calculate it); Scenario E (current guest — `GET /guest/queue-entries/current` returns the correct position for the authenticated guest); Scenario F (other statuses — position remains absent/null for READY/SEATED/LEFT/NO_SHOW); Scenario G (no side effects — calling the endpoint must not create `SeatingAssignment`/`SeatingAssignmentTable` or modify table state).
>
> **§10 Regression tests.** Run the complete backend suite. Existing allocation tests must continue to pass — especially verify allocation candidate selection unchanged, starvation behavior unchanged, combined table allocation unchanged, concurrency tests unchanged, Staff Queue unchanged unless the specification requires its position semantics to change. Do not weaken or delete existing tests.
>
> **§11 Real verification.** Use the real Docker/PostgreSQL application. Create a controlled scenario with at least two waiting guests whose chronological order differs from their current eligibility: Guest A joins first, Guest B joins second, their current eligibility differs, `GET /guest/queue-entries/current` for each returns positions per the specification, changing table availability changes the reported position when the spec says it should, and no new reservation is created by the GET request. Also verify a starvation scenario if it can be reproduced safely with controlled timestamps/test data. Clean up only disposable verification data — do not touch unrelated pre-existing rows.
>
> **§12 Browser verification.** Use the existing Guest UI. Verify Guest A → WAITING → position shown from the real API, and Guest B → WAITING → position shown from the real API. Confirm the UI displays the backend's calculated position, not a locally-calculated one. Do not redesign the Guest UI.
>
> **§13 Documentation.** Update only the documentation necessary to reflect the corrected position semantics — likely `api-spec.md`, `traceability.md`, relevant AI working-record files. Do not rewrite unrelated documentation.
>
> **§14 Strict scope.** DO NOT implement: Staff Release/No-show/Table/Login changes, Guest Leave changes, new allocation algorithms, Redis, Sidekiq, WebSockets, notifications, live updates, analytics, P1 fairness features, predictive demand, AI runtime allocation, new authentication, new database tables, frontend redesign, unrelated refactoring. This task is ONLY: make Guest queue position comply with the existing P0 specification.
>
> **§15 Git.** Before committing: `git status`, `git diff --stat`, `git diff -- <relevant files>`. Run `npx tsc -b --noEmit` if frontend files changed. Run the full backend test suite. Create ONE focused commit: `fix: make guest queue position spec compliant`. Do NOT push automatically. Leave the working tree clean.
>
> **§16 Final response:** report concisely — exact position rule implemented; existing specification/decision used as authority; whether Staff Queue position was changed and why; tests added; full suite result; real API verification; browser verification; database/no-side-effect verification; documentation changed; commit SHA; confirmation that allocation scoring/behavior was not changed; confirmation that no Redis/Sidekiq/P1 work was added. "STOP HERE. Do not start the next P0 task."

---

## Prompt 30 — P0 Final Audit: Requirements, Consistency, Verification, Release Readiness

**Source:** `P0_Final_Audit_Claude_Prompt(1).md` (candidate-authored, read from `~/Downloads/`), delivered complete in a single message. Explicitly framed as "an audit and correction task, NOT a new feature-development phase" — the first task in this project's history to require a comprehensive re-verification of everything built across the prior 29 prompts, rather than authorizing new implementation.

**Full text (condensed; all numbered sections' substantive content preserved):**

> **Objective.** Verify the implemented system satisfies the authoritative P0 requirements, identify remaining P0 gaps or inconsistencies, make only necessary P0 corrections, and prepare the repository for a P0-complete checkpoint.
>
> **§1 Audit first — do not modify code immediately.** Read `CLAUDE.md`, `functional-requirements.md`, `traceability.md`, `functional-spec.md`, `api-spec.md`, `allocation-spec.md`, `allocation-algorithm.md`, `seating-allocation-policy.md`, relevant decision records (especially DEC-005 and DEC-014/INV-015), current backend/frontend implementation, current test suite, and Git history — so the audit is based on the actual current implementation, not previous session reports. Do not assume previous reports are authoritative; the repository specifications and actual code are.
>
> **§2 Build a P0 requirement matrix.** Internally map every P0 requirement to: requirement ID, specification location, backend implementation, frontend implementation (if applicable), automated tests, real verification evidence, status (PASS/GAP/DOCUMENTATION-ONLY). Cover at minimum, across Guest (join, validation, oversized rejection, idempotency, current status, WAITING position, READY code, leave, refresh/recovery, terminal states), Allocation (configuration generation, single/combined-table allocation, adjacency, compatibility, scarcity, starvation, tie-breaking, transactional reservation, locking, orchestration, no partial combined reservation, rollback, lazy expiration), Staff (login, auth enforcement, session persistence, Queue, Tables, Seat, Release, No-show, combined-table release, release-triggered allocation, ready/waiting no-show, no-show terminal behavior), Frontend (every P0 screen/action, real API integration, required loading/error/success states), and Database/invariants (migrations, table uniqueness, active-assignment exclusivity, historical preservation, combined-table atomicity, no unintended schema additions, no QueueEntry for impossible groups).
>
> **§3 Hard-path audit.** Verify at minimum: same-table allocation race; combined-table allocation race; combined-table atomic rollback; idempotent Guest Join; starvation protection; ready expiration; Guest Leave while waiting/ready; Staff No-show while waiting/ready; Staff Release of a seated guest; combined-table Staff Release; impossible/oversized group rejection; a valid group with currently-unavailable tables remains WAITING; dynamic Guest position; Staff authentication boundary; Guest token cannot authenticate as Staff; Guest/Staff session persistence; refresh does not create duplicate QueueEntries; GET/read operations create no allocations/assignments. If a required hard path is missing, determine whether it needs a test only, a documentation correction, or a code correction — do not automatically add new functionality.
>
> **§4 API contract audit.** Compare every implemented P0 endpoint against `api-spec.md` (method, route, auth, request/response body, status codes, error format, terminal-state behavior, idempotency behavior). Verify no undocumented P0 endpoints exist. If implementation and specification disagree: prefer the authoritative locked domain/specification, make the smallest correction, and update documentation only if the implementation is intentionally the final approved behavior. Do not invent new API behavior.
>
> **§5 Frontend ↔ backend audit.** For every P0 UI action, verify UI → real HTTP → real backend → real PostgreSQL state → UI renders the backend's real result. Confirm the frontend fabricates nothing (position, seating status, seating code, table state, successful seat/release/no-show results). Verify Guest refresh (same visit, same real state via GET current) and Staff refresh (session restored, real API fetch) behavior; invalid/expired stored tokens must follow existing recovery behavior. Do not add a new `whoami` endpoint.
>
> **§6 Database/side-effect audit.** Run the real application against PostgreSQL. Confirm read endpoints don't unexpectedly create/allocate/release/modify state; confirm 40 seeded tables and 19 adjacency pairs remain, historical assignment rows are preserved, active table claims remain unique, combined assignments remain atomic. Do not delete pre-existing unrelated records; use disposable verification data and clean it up.
>
> **§7 Security/repository audit.** Run `git status`, `git ls-files`, the secret-pattern grep, `git check-ignore -v backend/config/master.key`. Confirm no real secret/token/browser-session-token/personal credential is tracked, the ignored Rails master key remains ignored, and existing dev-only credentials are clearly documented as local/dev values. Do NOT print secret values in the final report.
>
> **§8 Test suite.** Run the full existing backend suite and the existing frontend checks (`npx tsc -b --noEmit`, configured linter). Do not introduce a new test framework, weaken, delete, or bypass tests. If a pre-existing test is genuinely invalid because finalized domain behavior changed, make the smallest correction and document why.
>
> **§9 Real browser E2E verification.** Use the actual Docker stack and a real browser to verify the complete P0 journey: Guest flow (open → Join → WAITING/READY → correct position/code → refresh → recovered → Leave → terminal → refresh → no auto-rejoin); Staff flow (Login → Queue → Tables → READY guest → Seat → OCCUPIED → Release → FREE); No-show flow (WAITING → No-show → NO_SHOW; READY → No-show → NO_SHOW → released → available); Combined-table flow (large group → two adjacent tables → READY → Seat → both occupied → Release → both free); Impossible group (> maximum → 422 → no QueueEntry); Invalid authentication (missing/invalid Staff token → 401; Guest token against Staff API → 401). Do not expose real token values in the final report.
>
> **§10 Documentation consistency.** Audit documentation against the actual final implementation. Pay particular attention to stale statements: features described as "not implemented" when they now are; position described as deferred when dynamic position now exists; release/leave triggers described as unintegrated when implemented; frontend flows marked incomplete when now verified. Update only stale or incorrect documentation. Do NOT rewrite the entire documentation history; preserve historical session records where they accurately described the state at that time. Update current-state documents (`traceability.md`, `api-spec.md`); update AI working-record files only per established convention.
>
> **§11 P0/P1 boundary audit.** Confirm no out-of-scope P1 work (Redis, Sidekiq, WebSockets, live updates, notifications, analytics, rate limiting, predictive demand, AI runtime allocation, advanced staff management, OAuth/SSO/MFA, password reset, unrelated infrastructure) was introduced or expanded. Do not add any of these during this audit.
>
> **§12 Correction rules.** If everything passes: do not change code. If a P0 gap exists: fix only that gap, smallest possible change, add focused tests, re-run the full suite, re-run relevant real verification. If documentation-only: update documentation, do not modify implementation. If a finding belongs to P1: do not implement it, record it as a P1 follow-up. Do not turn this audit into another feature-development phase.
>
> **§13 Final acceptance criteria.** P0 complete only if: all P0 requirements PASS; all required P0 APIs implemented and verified; all required P0 frontend flows work; dynamic Guest position is compliant; impossible groups rejected before QueueEntry creation; Guest/Staff session persistence works; Staff Seat/Release/No-show work; combined-table operations remain atomic; allocation behavior remains deterministic and unchanged; hard-path tests pass; full suite passes; real Docker/PostgreSQL verification passes; browser E2E verification passes; no unintended secrets tracked; documentation internally consistent; no P1 functionality introduced. If any P0 item cannot be verified, mark it BLOCKED rather than claiming PASS.
>
> **§14 Git.** Before changes: `git status`, `git log --oneline --max-count=5`. After any changes: `git diff --stat`, `git diff`, `git status`. Run all required tests/verification before committing. If corrections are required, create ONE focused commit: `chore: complete p0 audit`. If no changes are required, do NOT create an empty commit. Do NOT push automatically. Leave the working tree clean.
>
> **§15 Final output — exact format required:** `## P0 Status` (`COMPLETE` or `BLOCKED`); `## Requirement Audit` (a compact table: Area | Status | Evidence, covering Guest/Allocation/Staff/Frontend/Database/Hard paths/Security/Documentation); `## Findings` (only actual remaining issues, or "No remaining P0 gaps found."); `## Corrections` (only changes made during this audit, or "No implementation corrections required."); `## Verification` (backend test result, frontend type/lint result, real API verification, real browser verification, PostgreSQL verification, security/repository verification); `## Git` (current commit, whether a new audit commit was created, working tree status); `## P1 Deferred` (significant P1 items explicitly deferred). "STOP. Do not start any new feature or P1 phase after this audit."
