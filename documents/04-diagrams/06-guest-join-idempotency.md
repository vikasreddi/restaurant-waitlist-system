...
Notes:
- The unique constraint on `queue_entries.idempotency_key` (`03-architecture/data-model.md` — a column on `QueueEntry` itself, not a separate `idempotency_records` table; that was a Phase 3 draft, revised in Phase 5B.1) is the enforcement point — this is a database guarantee, not a frontend-only debounce (guards against the AI-agent risk of "idempotency implemented only in the frontend," Phase 1 analysis §20).
