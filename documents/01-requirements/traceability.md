...
| REQ-TABLE-004 (smaller group, larger table) | `allocation-spec.md` §1, `allocation-algorithm.md` §6 (`fit_score`) | `backend/app/services/allocation/decision_engine.rb` — decision logic implemented (Phase 5B.5.2); not yet reachable by any real guest/table event (no trigger wiring, Phase 5B.5.4+) | TEST-023, plus `decision_engine_test.rb`'s fit-score tests |
| REQ-TABLE-005 (max two-table combination) | DEC-002 | — | TEST-006 |
| REQ-TABLE-006 (atomic combined allocation) | `allocation-spec.md` §5, `allocation-algorithm.md` §18, INV-005 | `backend/app/services/allocation/reservation_service.rb` — real `SELECT ... FOR UPDATE` locking, post-lock re-check, atomic 1–2-row `SeatingAssignmentTable` creation, verified under real PostgreSQL concurrency (Phase 5B.5.3); not yet triggered by any guest/staff event | TEST-005, TEST-006, plus `reservation_service_test.rb`/`reservation_service_concurrency_test.rb` |
| REQ-TABLE-007/008 (combination lifecycle) | INV-004, INV-006 | — | TEST-022, TEST-008 |
| REQ-QUEUE-001/002 (not FIFO, position reflects availability) | `seating-allocation-policy.md`, `allocation-algorithm.md` (full document) | `backend/app/services/allocation/{decision_engine,reservation_service}.rb` — decision + transactional reservation both implemented (Phase 5B.5.2/5B.5.3); no automatic trigger yet (join/release/no-show/leave integration is Phase 5B.5.4+), so no real waiting entry is allocated by this code outside of a direct, manual service call | TEST-011, TEST-024 |
| REQ-QUEUE-003 (starvation protection) | `starvation-policy.md`, `allocation-algorithm.md` §9 | `backend/app/services/allocation/decision_engine.rb` — derived (never stored) starvation protection with categorical pool override, implemented and tested (Phase 5B.5.2); not yet reachable by any real event | TEST-009, TEST-010 |
| REQ-QUEUE-004 (dynamic position) | DEC-005, `allocation-algorithm.md` §11 (`total_score`) | Guest-facing `position` (Phase 5B.4) remains the chronological-rank approximation — the full `total_score`-based computation exists in `decision_engine.rb` but is not wired into any guest-facing read yet | TEST-011 |
