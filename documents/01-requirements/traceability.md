...
| REQ-FE-001–006 (frontend) | `functional-requirements.md` | `frontend/src/App.tsx` — **guest join + leave screen only**: form → `POST /guest/queue-entries` → `GET /guest/queue-entries/current` → renders WAITING+position or READY+seating_code, with a "Leave Queue" button → `POST /guest/queue-entries/current/leave` → renders LEFT; loading/error states handled throughout. No staff frontend, no live updates — all other frontend requirements remain unimplemented | manual — real-browser end-to-end verification (no frontend test framework exists in this project yet); TEST-003 (double-join, backend-level) |
